...
void FeatureMatcher::setDetecter(const std::string &detType) {
    if (detType=="orb")        detector=cv::ORB::create(500);
    else if (detType=="brisk") detector=cv::BRISK::create();
    else if (detType=="akaze") detector=cv::AKAZE::create();
    else if(detType=="surf" )  detector=cv::xfeatures2d::SURF::create(500);
    else if(detType=="sift" )  detector=cv::xfeatures2d::SIFT::create(500);
    else throw std::runtime_error("Invalid descriptor");
    assert(!detType.empty());
}
